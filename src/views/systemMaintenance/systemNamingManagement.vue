  ...
  <div class="app-container">
    ...
    <el-table v-loading="listLoading" :data="list.slice((currentPage-1)*pagesize,currentPage*pagesize)" border fit highlight-current-row style="width: 100%">
      ...
      <el-table-column align="center" label="所属者" width="auto">
        <template slot-scope="scope">
          <span>{{ `${scope.row.ownerName}(${scope.row.owner})` }}</span>
        </template>
      </el-table-column>
    ...
    <el-dialog :visible.sync="newRemoteFormVisible">
      <el-form ref="newRemoteForm" :model="dialogTemp" :rules="newRemoteRules" label-position="left" label-width="90px" style="width: 400px; margin-left:50px;">
        ...
        <el-form-item label="业务系统描述" prop="description">
...
<script>
import { fetchBusinessSystems, addBusinessSystem, modifyBusinessSystem, deleteBusinessSystem } from '@/api/system/business_systems'
  ...
  methods: {
    ...
    handleDelete(row) {
      this.$confirm('是否确认删除 ' + row.name, '提示', {
        ...
        deleteBusinessSystem(row.id).then(_ => {
          this.getSystemList()
          this.$notify({
            title: 'Success',
            message: '删除成功',
    ...
    handleAddNewRemote() {
      this.$refs['newRemoteForm'].validate((valid) => {
        if (valid) {
          ...
          if (this.newOrEdit) {
            addBusinessSystem(tempData).then(() => {
              this.getSystemList()
            ...
            modifyBusinessSystem(tempData.id, tempData).then(() => {
              this.getSystemList()
              this.newRemoteFormVisible = false
              this.$notify({
                title: 'Success',
